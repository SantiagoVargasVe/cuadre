...
const base = { toUserId: "beto", currency: "COP", settledOn: "2026-08-20" };

describe("settlementFormSchema", () => {
  it("accepts any positive amount, including one over what's owed", () => {
    const schema = settlementFormSchema();
  ...
  it("requires a currency, and validates the amount against whichever one is chosen", () => {
    const schema = settlementFormSchema();
    expect(schema.safeParse({ toUserId: "beto", settledOn: "2026-08-20", amount: "1" }).success).toBe(false);
    expect(schema.safeParse({ ...base, currency: "us", amount: "1" }).success).toBe(false);
    expect(schema.safeParse({ ...base, currency: "USD", amount: "1" }).success).toBe(true);
  });
...
describe("toCreateInput", () => {
  it("converts the major-unit field to a minor-unit string at the field's own currency", () => {
    const values = { toUserId: "beto", currency: "COP", amount: "50.000", settledOn: "2026-08-20", note: "  " };
  ...
  it("keeps a real note, trimmed", () => {
    const values = { toUserId: "beto", currency: "USD", amount: "1", settledOn: "2026-08-20", note: "  cash  " };
    expect(toCreateInput(values).note).toBe("cash");
  });
});
