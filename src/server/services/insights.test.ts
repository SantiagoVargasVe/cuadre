...
describe.skipIf(!hasTestDatabase)("getInsights", () => {
  ...
  it("aggregates by day, month, member, and category", async () => {
    const { groupId, memberIds } = await seedGroup(["Ana", "Beto"]);
    ...
    // 120000 split equally between two members → 60000 each. Tied amounts,
    // so the order is the stable userId tiebreak — compare as a set.
    expect([...cop.byMember].sort((a, b) => (a.userId < b.userId ? -1 : 1))).toEqual(
      [
        { userId: ana, amount: "60000" },
        { userId: beto, amount: "60000" },
  ...
  it("converts and labels the block when a display currency is pinned", async () => {
    ...
    expect(block.byMember).toEqual([{ userId: memberIds[0]!, amount: "30000" }]);
  });
