...
- [x] `cloudflared` (which lives in a neighbouring stack's compose on the host, not its own) joins the
...
- [x] Host `.env` has `APP_URL=<the app's public origin>` (no trailing slash); it reaches
      the container via the compose `environment:` allowlist
- [x] Origin check verified against the live URL: `POST /api/auth/register` with a matching
      `Origin` → `400 VALIDATION_ERROR` (past the gate);
...
- Created the deploy directory — `docker-compose.yml` (copy of `infra/docker-compose.prod.yml`), `.env`
...
- Added the `cuadre` external network to the neighbouring stack's `cloudflared` service
...
- Installed `cuadre-deploy.{service,timer}` and `cuadre-fx.{service,timer}` +
  `/usr/local/bin/cuadre-fx-refresh` + `/etc/cuadre/fx-refresh.token` (chmod 600, owned by the
  deploy account — the service runs as that account, not root). Both timers `enabled`.
