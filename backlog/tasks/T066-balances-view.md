...
- [x] Per-member summary: paid, share, net — sign and word, never colour alone
- [x] The payment plan: who pays whom, how much
- [x] Simplify is a `Switch` that **`PATCH`es the group** and re-renders. Not a client-side
      transform, and no local "simplified" state to drift
- [x] **A simplified edge is explainable.** Tapping a payment shows the raw debts it replaced,
...
- [x] **Never a negative amount as a direction.** "Ana te debe $ 20.000" and "le debes a Ana
      $ 20.000" are different sentences, not one number with a sign
- [x] Multi-currency: **one block per currency**, separate headings, no combined total, and no
      layout that implies one
- [x] With a display currency set, amounts are labelled and the pin's date and source are reachable
      in one tap
- [x] Balances are **always from `GET /api/groups/:id/balances`**, never computed client-side from
      the feed
- [x] A settled group reads as settled — a calm zero state, not an empty table
- [x] Every amount has an accessible label naming whose it is
- [x] Tests: toggling calls the `PATCH` and re-renders; `explains` renders on tap; a mixed-currency
