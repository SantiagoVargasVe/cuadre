...
- [x] `src/lib/money/format.ts` is the **only** place `Intl.NumberFormat` is called. Add a lint
      rule if that's cheap
- [x] **COP formats with `maximumFractionDigits: 0`.** Verified: the default
...
- [x] **EUR under `es-CO` renders `EUR 45,00`, not `€45,00`.** Verified. Choose the locale per
      currency or set `currencyDisplay` explicitly
- [x] `<Money value={Money} signed? />` — `signed` renders `+`/`−` for net positions
- [x] `font-variant-numeric: tabular-nums` wherever money appears in a column, or a balances list
...
- [x] Credit/debit colouring uses `text-credit` / `text-debit`, **never** `text-destructive` —
...
- [x] A converted amount is **always marked**, with the original and the pin date reachable. An
      unlabelled converted number is a trust bug
- [x] `<MoneyField>`: locale thousands separators while typing (`150.000`), `inputMode="decimal"`,
      and a **`bigint` of minor units out of `onSubmit`**
- [x] The major→minor conversion happens **once, at the form boundary**. Nothing downstream sees a
      `Number`, and no other component parses money
- [x] Tests: COP renders without decimals; USD and EUR render with them; EUR under `es-CO` matches
