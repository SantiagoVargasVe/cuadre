...
- [x] `GET /api/groups/:id/expenses.csv` returns every live expense the requester may read, after
...
- [x] The response is UTF-8 `text/csv; charset=utf-8` with `Content-Disposition: attachment` and a
      filename derived from the group title but safely sanitized. It is a download, never JSON
- [x] The header is exactly `expense_id,date,title,amount_minor,currency,split_strategy,payers,splits,created_at,updated_at`.
...
- [x] Serialize RFC 4180 CSV correctly (commas, quotes, CR/LF, and Unicode). Protect spreadsheet
...
- [x] Results are ordered `expense_date ASC, id ASC` so two exports of unchanged data have the same
      row order. Deleted expenses are excluded through `liveExpenses`; settlements are not expenses
      and never appear
- [x] Use a bounded-query service, not a request per expense, payer, or split. Do not reuse the
      paginated feed endpoint by looping over cursors
- [x] Multi-currency rows retain their individual currencies. The export has no converted column,
      display-currency setting, aggregate total, or implied cross-currency sum
- [x] Add an i18n-backed, accessible "Exportar CSV" download control on the group expense view;
      it is available for an empty group too, whose CSV contains only the header. It must not block
      the common expense-entry flow
- [x] Tests: membership and removed-member `404`; content type/disposition; exact header and stable
...
- [x] `docs/context/api-contract.md` documents the route, response headers, CSV columns, and its
