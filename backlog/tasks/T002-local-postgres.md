...
- [ ] `infra/docker-compose.dev.yml` with `postgres:17-alpine`, a named volume, and a healthcheck
- [ ] `infra/postgres-init/01-create-test-db.sql` creating `cuadre_test` on a fresh volume —
      integration tests need it and it must exist before anyone writes one
- [ ] `db:up` waits for **healthy**, not just started. `db:down`, `db:logs`, `db:reset`
- [ ] All `db:*` scripts pass `--project-directory .` so `.env` resolves from the repo root —
      without it compose silently reads `infra/.env` and the failure is baffling
- [ ] `src/server/config.ts` validates the environment **once at boot** with Zod and fails with a
      message naming exactly what is wrong. No defaults for secrets
- [ ] `AUTH_SECRET` is rejected below 32 characters rather than accepted and weak
- [ ] `SUPPORTED_CURRENCIES` parses to a non-empty set, and `DEFAULT_CURRENCY` must be in it
- [ ] `src/server/config.schema.ts` holds the schema separately, importable by tooling that runs
      outside Next (`drizzle.config.ts`) without tripping the `server-only` guard
- [ ] `config.ts` carries `import "server-only"`; nothing under `src/app/**` can import it
- [ ] Tests: a missing required var fails with its name in the message; a short `AUTH_SECRET` is
