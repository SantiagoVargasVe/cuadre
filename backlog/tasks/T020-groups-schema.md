...
- [ ] `currencies`: `code char(3) pk`, `exponent smallint`, `display_decimals smallint`, `name`.
      Seeded by migration with COP, USD, EUR
- [ ] **COP seeds as `exponent 2, display_decimals 0`.** They differ on purpose — ISO gives COP two
      minor digits and Colombians write none. See
      [ADR-0004](../../docs/adr/0004-money-as-integer-minor-units.md)
- [ ] `groups`: `id uuid pk`, `title`, `description`, `default_currency → currencies`,
      `display_currency → currencies` **nullable**, `simplify_debts boolean default false`,
      `created_by`, `archived_at` nullable, timestamps
- [ ] `group_members`: composite pk `(group_id, user_id)`, `role` (`owner | member`), `joined_at`,
      `removed_at` nullable
- [ ] **An explicit `UNIQUE (group_id, user_id)`** even though it is the primary key — child tables
...
- [ ] Add the deferred `invite_codes.group_id` FK left open by T010
- [ ] Index on `group_members(user_id) WHERE removed_at IS NULL` — "my groups" is the app's most
      frequent query
- [ ] Members are **never hard-deleted**; `removed_at` retires them. Historical expenses reference
      the row, so it must survive
- [ ] Migration reviewed and committed with the schema
