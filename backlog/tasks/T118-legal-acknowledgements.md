...
- [x] Publish public, self-hosted `/terms` and `/privacy` routes. They render without a session,
...
- [x] Add a single source of truth for the two document identifiers and their current version
...
- [x] Add a `legal_acceptances` relation rather than consent booleans on `users`. Each immutable
...
- [x] The migration backfills the two initial-version records for every account that exists when
...
- [x] Add separate, required Terms and Privacy controls to `/register`, each with an accessible
...
- [x] Extend the shared registration schema and `POST /api/auth/register` payload with two
...
- [x] A successful registration inserts both explicit acknowledgement records in the same database
...
- [x] Document the new relation and retention semantics in `data-model.md`, the expanded register
...
- [x] Tests cover both public document routes, distinct accessible checkboxes and links, individual
