...
- [ ] `apportion(total: bigint, weights: Map<Id, bigint>, seed: string): Map<Id, bigint>`
- [ ] Exact: `Σ output == total`, always, for every input. This is not a tolerance
- [ ] **Largest remainder method** — floor each share, then distribute the leftover units to the
      largest fractional remainders. Remainders are compared as exact integers, never as floats
- [ ] **Ties break by rotation, not by member id.** With an `equal` split every remainder is
...
- [ ] Deterministic: the same `(total, weights, seed)` produces byte-identical output every time.
...
- [ ] The six strategies, all resolving to `(memberId, amount)` pairs summing exactly to the total:
...
- [ ] `percentage` rejects a basis-point sum ≠ `10000` with the actual sum in `details`
- [ ] `exact` rejects with `expected`, `actual`, and `difference` in `details` — the split editor
      renders that difference live, so it must be structured, not a message
- [ ] Weights of zero are rejected. A member with a zero share should not be in the split at all
- [ ] Tests per [splitting.md](../../docs/context/splitting.md) § 8, including the worked example:
