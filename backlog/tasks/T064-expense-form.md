...
- [ ] Full-screen sheet on mobile, dialog on desktop, with **the amount field focused on open**
- [ ] Everything pre-filled: currency → the group's default, date → today, paid by → you for the
      full amount, split → every member equally
- [ ] **Title, amount, save.** That path must require no other interaction
- [ ] "Pagado por" and "Dividido" are **two lines of text that open editors when tapped** — not two
      always-open pickers
- [ ] Amount uses `<MoneyField>`; `bigint` minor units out at submit
- [ ] Currency selector limited to `SUPPORTED_CURRENCIES`
- [ ] The payer editor supports **multiple payers with amounts**, showing a live remainder against
      the total, and defaults to a single payer
- [ ] Save disabled while the form is invalid or in flight
- [ ] **Never optimistic.** The server resolves the split; guessing its answer is how client and
      server end up disagreeing about who owes what
- [ ] On success, invalidate the group's expenses **and balances** query keys. A stale balance after
      an add is the most damaging wrong number this app can show
- [ ] API `422`s render against the right control, using `details` rather than a generic message
- [ ] Tests: the default path submits the documented minimal payload; multi-payer submits and shows
