...
- [ ] `requireMembership(groupId, userId)` → returns the membership row, or throws `NotFoundError`
- [ ] `requireOwner(groupId, userId)` → throws `ForbiddenError` if a member but not `owner`
- [ ] **`removed_at IS NOT NULL` is not a member.** A removed member loses access immediately
- [ ] **Non-membership is `404`, not `403`.** Groups are private and their ids are unguessable;
      ...
      so nobody "fixes" it
- [ ] An archived group is readable but rejects writes, with a distinct error code
- [ ] Both guards live in `src/server/auth/` and are called **inside services**, never in route
      handlers. Document that at the definition
- [ ] A helper for the id-addressed case: given an expense or settlement id, load the row, read
      *its* `group_id`, then check. This is where the check gets forgotten, so make the easy path
      the correct one
- [ ] Tests: non-member `404`; removed member `404`; member-not-owner `403` on an owner action;
